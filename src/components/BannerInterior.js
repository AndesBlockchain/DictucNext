...
const BannerInterior = ({ banner, gatsbyImageData, fallback, alt = "Banner", titulo = "", titulo_visible = true, color_titulo = "azul-dictuc", icono_secundario = "" }) => {
  ...
  return (
    <div className="w-full pt-2 mb-6 relative">
      ...
      {icono_secundario && (
        <div className="absolute -bottom-5 right-0 z-10 bg-gris-dictuc shadow-md rounded-tl-lg px-5 py-4">
          <img src={process.env.STRAPI_API_URL + icono_secundario} className="max-w-10" alt="Icono secundario" />
        </div>
      )}
    </div>
