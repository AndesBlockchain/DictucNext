...
export default function Contacto({ titulo = "¿En qué podemos <span class='text-azul-dictuc'>ayudarte?</span>", border = false, isCotizacion=false, servicio="", tiposDeContacto, strapiApiUrl, codigoUnidad, accionInicial = null, nombreServicio}) {
  ...
  return (
    <div className={`w-full col-md-8 mt-5 rounded-xl p-6 ${border ? 'border' : ''}`}>
      ...
      <fieldset className="fieldset">
          <div className="join"><input type="checkbox" disabled={isConsultingRut} {...register("politica", {required:true})}/> &nbsp;Acepto la&nbsp;<a className="font-semibold" href="https://backendweb.dictuc.cl/uploads/Politica_de_Tratamiento_de_Datos_Personales_Dictuc_406721e9e7.pdf" target="_blank">&nbsp;política de tratamiento de datos</a></div>
